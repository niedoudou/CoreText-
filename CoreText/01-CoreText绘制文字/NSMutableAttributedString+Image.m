...
{
    // 1.获取imageData
    SXTAttributedImage *imageData = (__bridge SXTAttributedImage *)ref;
 ...
 * 获取占位图片的最终大小
 */
static CGSize attributedImageSize(SXTAttributedImage *imageData)
{
 ...
 * 获取图片占位的属性字符串
 */
+ (NSAttributedString *)attributedStringWithImageData:(SXTAttributedImage *)imageData
{
                              ...
                              font:(UIFont *)font
{
    ...
    [regex enumerateMatchesInString:self.string options:0 range:NSMakeRange(0, self.string.length) usingBlock:^(NSTextCheckingResult * _Nullable result, NSMatchingFlags flags, BOOL * _Nonnull stop) {
        ...
        // 3.1初始化并设置图片对象属性
        SXTAttributedImage *imageData = [SXTAttributedImage new];
        // 3.1.1获取图片名字
    ...
    for (SXTAttributedImage *imageData in arrM) {
        // 4.1获取图片
